...
const pizSliderArrows = document.querySelectorAll('.slider_button');
const pizSlider = document.querySelector('.slider_cards');
let pizOfset = 0;
let pizStap = 360;
...
let pizMaxWidth = (arr1.length - 3) * pizStap;
function cardCreate(param) {
  let cardDiv = document.createElement('div');
  cardDiv.className = 'slider_card';
  cardDiv.id = param.id
  cardDiv.innerHTML =
    `<img src="${param.imgSrc}" alt="${param.alt}" class="slider_card__avatar">
  <div class="slider_card__name">${param.name}</div>
  <div class="slider_card__button">Learn more</div>`
  document.querySelector('.slider_cards').appendChild(cardDiv);
...
if (window.innerWidth > 1279) {
  countCards = arr1.length;
}
if (window.innerWidth <= 1279 && window.innerWidth > 500) {
  countCards = arr1.length;
}
...
function generate() {
  document.querySelector('.slider_cards').innerHTML = '';
...
//Слайдер стрелки
pizSliderArrows.forEach(arrow => arrow.addEventListener('click', () => pizArrow(arrow)));

// pizSlider.style.width = (arr1.length * pizStap - 90) + 'px';
// pizSlider.style.width = (arr1.length * pizStap - 36,25) + 'px';

function pizArrow(arrow) {
  if(window.innerWidth <= 1100) {
      pizStap = 312;
      console.log(pizSlider)
      pizMaxWidth = (arr1.length - 2) * pizStap;
  }
  if(window.innerWidth <= 500) {
    pizStap = 312;
    pizMaxWidth = (arr1.length - 1) * pizStap;
}
  if(arrow.id === '2') {
      pizOfset += pizStap;
      if(pizOfset > pizMaxWidth) {
        pizOfset = 0;
      }
      pizSlider.style.left = -pizOfset + 'px';
      // console.log(pizOfset)
      
  } else if(arrow.id === '1') {
      pizOfset -= pizStap;
      if(pizOfset < 0) {
        pizOfset = pizMaxWidth;
      }
      pizSlider.style.left = -pizOfset + 'px';
  }
}
